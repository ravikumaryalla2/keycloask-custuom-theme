...
export default function UserProfileFormFields(props: UserProfileFormFieldsProps<KcContext, I18n>) {
    ...
    //         .replace(/\b\w/g, (chr: string) => chr.toUpperCase()); // Capitalize the first letter of each word
    // }
                        ...
                        >
                            ...
                            <div className={kcClsx("kcInputWrapperClass")}>
                                ...
                                {/* <ODSTextField
                                    ...
                                    fullWidth
                                    size="small"
                                    placeholder={toTitleCase(attribute.name)}
                                    sx={{ height: "40px" }}
                                /> */}
                                <InputFieldByType
                                    attribute={attribute}
        ...
        <>
            ...
            <ODSTextField
                ...
                helperText={displayableErrors
                    ?.filter(error => error.fieldIndex === fieldIndex)
                    ?.slice(0, 1)
                    ?.map(({ errorMessage }, i) => <Fragment key={i}>{errorMessage}</Fragment>)}
                id={attribute.name}
                name={attribute.name}
                value={(() => {
                    if (fieldIndex !== undefined) {
                ...
                disabled={attribute.readOnly}
                autoComplete={attribute.autocomplete}
                placeholder={advancedMsgStr(attribute.displayName ?? "")}
                fullWidth
                minLength={
                    attribute.annotations.inputTypeMinlength === undefined ? undefined : parseInt(`${attribute.annotations.inputTypeMinlength}`)
                }
